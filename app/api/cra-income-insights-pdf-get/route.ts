...
export async function POST(request: NextRequest) {
  try {
    ...
    // CRA Income Insights PDF uses user_id (new) or user_token (legacy)
    ...
    // Raw PDF binary: base64-encode and return { pdf: "..." }
    const arrayBuffer = await response.arrayBuffer();
    if (!response.ok) {
      return NextResponse.json(
        { error: 'Failed to get CRA income insights PDF' },
    ...
    console.error('Error getting CRA income insights PDF:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to get CRA income insights PDF' },
      { status: 500 }
    );
  }
}
